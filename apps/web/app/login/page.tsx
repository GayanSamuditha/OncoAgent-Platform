...
export default function LoginPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    const response = await fetch(`${apiBase}/api/v1/auth/login`, { method: "POST", credentials: "include", headers: { "content-type": "application/json" }, body: JSON.stringify({ user_key: userKey }) });
    if (!response.ok) { setMessage("Local identity was not accepted."); return; }
