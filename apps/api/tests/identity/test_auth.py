...
def test_cookie_state_change_requires_configured_origin() -> None:
    with TestClient(app) as client:
        assert (
            client.post("/api/v1/auth/login", json={"user_key": "researcher-console"}).status_code
            == 200
        )
        denied = client.post("/api/v1/auth/logout", headers={"Origin": "https://untrusted.example"})
        assert denied.status_code == 403
        allowed = client.post("/api/v1/auth/logout", headers={"Origin": "http://127.0.0.1:3000"})
