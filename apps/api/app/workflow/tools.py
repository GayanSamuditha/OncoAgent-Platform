...
class RegisteredTool:
    def __init__(
        self, descriptor: ToolDescriptor, request_model: type[BaseModel], handler: ToolHandler
    ) -> None:
...
def _descriptor(
    name: str, description: str, roles: list[str], maximum: int = 100
) -> ToolDescriptor:
    return ToolDescriptor(
        name=name,
        version="phase3a-tool-v1",
        description=description,
        allowed_roles=roles,
        read_only=True,
        timeout_seconds=10,
        maximum_result_size=maximum,
        retry_policy={"max_attempts": 2, "retryable": ["transient_read"]},
    )


def _rows(
    session: Session, model: type[Any], dataset_id: str, patient_id: str, maximum: int = 100
) -> list[dict[str, Any]]:
    rows = session.scalars(
        select(model)
        .where(model.dataset_id == dataset_id, model.patient_id == patient_id)
        .limit(maximum)
    ).all()
...
def _demographics(context: ToolExecutionContext, request: BaseModel) -> dict[str, Any]:
    ...
    patient = context.session.scalar(
        select(Patient).where(
            Patient.id == patient_request.patient_id,
            Patient.dataset_id == patient_request.dataset_id,
        )
    )
    if patient is None:
...
def _resource_handler(model: type[Any]) -> ToolHandler:
    return lambda context, request: {
        "items": _rows(
            context.session,
            model,
            PatientRequest.model_validate(request).dataset_id,
            PatientRequest.model_validate(request).patient_id,
        )
    }
...
def _date_window(_: ToolExecutionContext, request: BaseModel) -> dict[str, Any]:
    ...
    return {
        "status": "verified" if start <= value <= end else "not_verified",
...
def _build_evidence(context: ToolExecutionContext, request: BaseModel) -> dict[str, Any]:
            ...
            facts.append(
                {
                    ...
                    "source_fhir_resource_id": row.get("source_resource_id") or row.get("fhir_id"),
    ...
    return {
        ...
        "items": facts[:100],
        "evidence_status": "structured_facts_only",
    }
...
def build_tool_registry() -> dict[str, RegisteredTool]:
    ...
    return {
        "search_clinical_documents": RegisteredTool(
            _descriptor(
                "search_clinical_documents", "Generate bounded candidate documents.", roles, 50
            ),
            SearchRequest,
            _search,
        ),
        "get_patient_demographics": RegisteredTool(
            _descriptor("get_patient_demographics", "Read synthetic patient demographics.", roles),
            PatientRequest,
            _demographics,
        ),
        "get_patient_conditions": RegisteredTool(
            _descriptor("get_patient_conditions", "Read normalized condition facts.", roles),
            PatientRequest,
            _resource_handler(Condition),
        ),
        "get_patient_observations": RegisteredTool(
            _descriptor("get_patient_observations", "Read normalized observation facts.", roles),
            PatientRequest,
            _resource_handler(Observation),
        ),
        "get_patient_procedures": RegisteredTool(
            _descriptor("get_patient_procedures", "Read normalized procedure facts.", roles),
            PatientRequest,
            _resource_handler(Procedure),
        ),
        "get_patient_medications": RegisteredTool(
            _descriptor(
                "get_patient_medications", "Read normalized medication request facts.", roles
            ),
            PatientRequest,
            _resource_handler(MedicationRequest),
        ),
        "get_patient_diagnostic_reports": RegisteredTool(
            _descriptor(
                "get_patient_diagnostic_reports", "Read normalized diagnostic report facts.", roles
            ),
            PatientRequest,
            _resource_handler(DiagnosticReport),
        ),
        "get_patient_encounters": RegisteredTool(
            _descriptor("get_patient_encounters", "Read normalized encounter facts.", roles),
            PatientRequest,
            _resource_handler(Encounter),
        ),
        "verify_date_window": RegisteredTool(
            _descriptor(
                "verify_date_window",
                "Verify an ISO timestamp against an explicit date window.",
                roles,
            ),
            DateWindowRequest,
            _date_window,
        ),
        "build_patient_evidence": RegisteredTool(
            _descriptor(
                "build_patient_evidence",
                "Assemble provenance-linked evidence from structured facts.",
                roles,
            ),
            PatientRequest,
            _build_evidence,
