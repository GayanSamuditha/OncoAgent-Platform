...
def intake(state: WorkflowState) -> dict[str, Any]:
    ...
    if not settings.agent_execution_enabled:
        policy(
            ...
            "kill_switch",
            "deny",
            "Agent execution is disabled by platform policy.",
        )
...
def create_plan(state: WorkflowState) -> dict[str, Any]:
    ...
    try:
        criteria = [
            ...
            for item in state.get("structured_input", {}).get("criteria", [])
        ] or None
        ...
        if requested_provider in {"auto", "qwen_local"}:
            ...
            try:
                outcome = local_provider.generate_cohort_plan(
                    state["original_request"], state["dataset_id"], criteria, max_candidates
                )
                ...
                event(
                    ...
                    "planner_fallback",
                    "create_plan",
                    {"from": "qwen_local", "to": "deterministic", "reason": exc.category},
                )
    ...
    lineage(
        ...
        "planner",
        planner_provider,
        str(planner_lineage.get("prompt_version", "phase3a-planner-v1")),
        planner_lineage | {"plan_version": plan.plan_version},
    )
    return {
        "structured_plan": plan_dict,
        ...
        "planner_lineage": planner_lineage,
        "requested_criteria": [item.model_dump(mode="json") for item in plan.criteria],
        "retrieval_policy": {
            "primary": "medcpt",
            "fallbacks": ["bioclinicalbert", "postgres_fts"],
            "reranker": "none",
        },
        "run_status": "validating_plan",
...
def policy_precheck(state: WorkflowState) -> dict[str, Any]:
    ...
    policy(
        ...
        "precheck",
        "allow",
        "Bounded read-only plan and synthetic dataset accepted.",
        {"max_candidates": state["structured_plan"]["max_candidates"]},
    )
...
def retrieve_candidates(state: WorkflowState) -> dict[str, Any]:
    ...
    for provider_name in ["medcpt", "bioclinicalbert", "postgres_fts"]:
        ...
        try:
            if provider_name == "postgres_fts":
                with SessionLocal() as session:
                    results, latency = postgres_fts_search(
                        session,
                        state["dataset_id"],
                        plan["retrieval_query"],
                        int(plan["max_candidates"]),
                        ["encounter", "patient-summary"],
                        None,
                    )
            else:
            ...
            attempts[-1].update(
                {"status": "success", "latency_ms": latency, "candidate_count": len(candidates)}
            )
            if provider_name != "medcpt":
                fallbacks.append(
                    {
                        "from": "medcpt" if provider_name == "bioclinicalbert" else "dense",
                        "to": provider_name,
                        "reason": last_error or "previous provider unavailable",
                    }
                )
            tool_call(
                state["run_id"],
                ...
                "success",
                {
                    ...
                    "query_length": len(plan["retrieval_query"]),
                    "top_k": plan["max_candidates"],
                },
                {"provider": provider_name, "candidate_count": len(candidates)},
                fallbacks[-1]["reason"] if fallbacks else None,
            )
            break
    ...
    if (
        ...
        return _fail(
            {**state, "retrieval_attempts": attempts, "retrieval_fallbacks": fallbacks},
            f"all retrieval providers failed: {last_error}",
        )
    save_candidates(state["run_id"], state["dataset_id"], candidates)
    return {
        "retrieval_attempts": attempts,
        "retrieval_fallbacks": fallbacks,
        "candidate_results": candidates,
        "candidate_patient_ids": [str(item["patient_id"]) for item in candidates],
        "candidate_document_ids": [str(item["document_id"]) for item in candidates],
        "run_status": "verifying",
        "current_node": "retrieve_candidates",
        "updated_at": _utc(),
    }
...
def _concept_match(item: dict[str, Any], criterion: dict[str, Any]) -> bool:
    ...
    return bool(
        (concept and concept in haystack)
        or synonym_match
        or (code and code == str(item.get("code") or "").lower())
    )
...
def structured_fhir_verification(state: WorkflowState) -> dict[str, Any]:
                ...
                try:
                    ...
                    result = (
                        execute_tool(
                            ...
                            tool_name,
                            context,
                            {"dataset_id": state["dataset_id"], "patient_id": patient_id},
                        )
                        if tool_name != "verify_date_window"
                        else {"items": []}
                    )
                    items = result.get("items", [result])
                    if criterion["criterion_type"] in {"minimum_age", "maximum_age", "gender"}:
                        demographic = execute_tool(
                            registry,
                        ...
                        if criterion["criterion_type"] == "gender":
                            matched = (
                                str(demographic.get("gender") or "").lower()
                                == str(criterion.get("value") or "").lower()
                            )
                            value = {
                                "gender": demographic.get("gender"),
                                "source_resource_id": demographic.get("source_resource_id"),
                            }
                        else:
                            threshold = int(criterion.get("value") or 0)
                            matched = age is not None and (
                                age >= threshold
                                if criterion["criterion_type"] == "minimum_age"
                                else age <= threshold
                            )
                            value = {
                                "age": age,
                        ...
                        matched_item = next(
                            (item for item in items if _concept_match(item, criterion)), None
                        )
                        matched = matched_item is not None
                        value = matched_item or {}
                    status = (
                        "verified" if matched else ("missing_data" if not items else "not_verified")
                    )
                    tool_call(
                        state["run_id"],
                        tool_name,
                        ...
                        },
                        {
                            ...
                            "source_present": bool(value.get("source_resource_id"))
                    ...
                    evidence.append(
                        {
                            ...
                            "criterion_description": str(
                                criterion.get("clinical_concept") or criterion.get("criterion_type")
                            ),
                            "verification_status": status,
                            "structured_value": value,
                            "source_resource_type": criterion["criterion_type"],
                            "source_fhir_resource_id": (
                                value.get("source_resource_id") if isinstance(value, dict) else None
                            ),
                            "encounter_id": value.get("encounter_id")
                            if isinstance(value, dict)
                            else None,
                            "effective_timestamp": None,
                            "explanation": "Verified against normalized structured synthetic FHIR facts."
                            if matched
                            else "Required structured fact was not verified.",
                            "verification_tool": tool_name,
                            "verification_tool_version": "phase3a-tool-v1",
                            "dataset_id": state["dataset_id"],
                        }
                    )
                    ...
                    evidence.append(
                        {
                            ...
                            "source_fhir_resource_id": None,
                            "encounter_id": None,
                            "effective_timestamp": None,
                            "explanation": f"Verification failed safely: {exc}",
                            "verification_tool": criterion.get("verification_tool", "unknown"),
                            "verification_tool_version": "phase3a-tool-v1",
                            "dataset_id": state["dataset_id"],
                        }
                    )
    ...
    return {
        ...
        "included_patient_ids": included,
        "excluded_patient_ids": excluded,
        "run_status": "validating_evidence",
        "current_node": "structured_fhir_verification",
        "updated_at": _utc(),
    }
...
def evidence_validation(state: WorkflowState) -> dict[str, Any]:
    ...
    required = {
        item["criterion_id"]
        for item in state["structured_plan"]["criteria"]
    ...
    for patient in state.get("included_patient_ids", []):
        ...
        if any(
            item["verification_status"] != "verified" or not item.get("source_fhir_resource_id")
            for item in rows
        ):
    ...
    return {
        ...
        "current_node": "evidence_validation",
        "updated_at": _utc(),
    }
...
def policy_postcheck(state: WorkflowState) -> dict[str, Any]:
    ...
    policy(
        ...
        "postcheck",
        "allow",
        "All required criteria are verified with structured provenance.",
        {"included_count": len(state.get("included_patient_ids", []))},
    )
    return {
        "run_status": "awaiting_approval",
        "current_node": "policy_postcheck",
        "updated_at": _utc(),
    }
...
def prepare_approval(state: WorkflowState) -> dict[str, Any]:
    ...
    payload = {
        ...
        "retrieval_fallbacks": state.get("retrieval_fallbacks", []),
        "candidate_count": len(state.get("candidate_patient_ids", [])),
        "included_patient_ids": state.get("included_patient_ids", []),
        "excluded_patient_ids": state.get("excluded_patient_ids", []),
        "evidence_summaries": [
            {
                key: item.get(key)
                for key in (
        ...
        "policy_reason": "Cohort finalization requires reviewer approval.",
        "requested_reviewer_actions": ["approve", "reject", "request_changes", "cancel"],
    }
    ...
    event(
        ...
        "awaiting_approval",
        "prepare_approval",
        {"approval_id": approval_id, "included_count": len(state.get("included_patient_ids", []))},
    )
    return {
...
def human_approval(state: WorkflowState) -> dict[str, Any]:
    if not get_settings().agent_execution_enabled:
        policy(
            ...
            "kill_switch",
            "deny",
            "Platform kill switch disabled continuation before approval.",
        )
    ...
    payload = {
        ...
        "original_request": state["original_request"],
        "structured_plan": state["structured_plan"],
        "retrieval_providers_used": [
            item.get("provider")
            for item in state.get("retrieval_attempts", [])
            if item.get("status") == "success"
        ],
        "fallback_events": state.get("retrieval_fallbacks", []),
        "candidate_count": len(state.get("candidate_patient_ids", [])),
        "proposed_included_patients": state.get("included_patient_ids", []),
        "proposed_excluded_patients": state.get("excluded_patient_ids", []),
        "criterion_level_evidence_summaries": [
            {
        ...
        "warnings": state.get("warnings", []),
        "policy_reason": "Reviewer approval is required before cohort finalization.",
        "requested_reviewer_actions": ["approve", "reject", "request_changes", "cancel"],
    }
    ...
    return {
        ...
        "approval_status": str(decision.get("decision", "unknown")),
    ...
    with SessionLocal.begin() as session:
        session.add(
            ApprovalDecision(
                ...
                actor_role=str(decision.get("actor_role")),
                decision="approve",
...
def reject_result(state: WorkflowState) -> dict[str, Any]:
    ...
    event(
        state["run_id"],
        state["run_id"],
        "rejected" if status == "rejected" else "request_changes",
        "reject_result",
        {"decision": decision.get("decision")},
    )
    return {
        "run_status": status,
        "current_node": "reject_result",
        "final_result": {
            "approval_id": state["approval_id"],
            "decision": decision.get("decision"),
            "comment": decision.get("comment"),
...
def record_completion(state: WorkflowState) -> dict[str, Any]:
    ...
    update_run(
        state["run_id"],
        status=status,
        current_node="record_completion",
        completed_at=datetime.now(UTC)
        if status in {"completed", "rejected", "cancelled", "failed", "needs_clarification"}
        else None,
...
def _after_validate(state: WorkflowState) -> str:
    return (
        "policy_precheck"
        if state.get("run_status") == "validating_plan" and not state.get("errors")
        else "fail_safely"
    )
...
def _after_retrieve(state: WorkflowState) -> str:
    return (
        "structured_fhir_verification" if state.get("run_status") == "verifying" else "fail_safely"
    )
...
def build_graph(checkpointer: BaseCheckpointSaver[str]) -> Any:
    ...
    for name, function in (
        ...
        ("structured_fhir_verification", structured_fhir_verification),
        ("evidence_validation", evidence_validation),
        ("policy_postcheck", policy_postcheck),
