...
def _schema_hash() -> str:
    return hashlib.sha256(
        json.dumps(CohortPlan.model_json_schema(), sort_keys=True).encode()
    ).hexdigest()
...
class OllamaQwenPlannerProvider:
    ...
    def __init__(
        self, settings: Settings, model_name: str | None = None, benchmark: bool = False
    ) -> None:
        self.settings = settings
        requested_model = (
            model_name or settings.local_planner_default_model or settings.local_llm_model
        )
        ...
        self.keep_alive = (
            settings.local_planner_benchmark_keep_alive
            if benchmark
            else settings.local_llm_keep_alive
        )
    ...
    def metadata(self) -> dict[str, Any]:
        return {
            ...
            "cohort_plan_schema_hash": _schema_hash(),
    ...
    def health(self) -> dict[str, Any]:
        payload: dict[str, Any] = {
            **self.metadata(),
            ...
            "healthy": False,
            "loaded": None,
            "last_successful_request": self._last_success,
            "last_failure_category": self._last_failure,
            "limitations": ["Local Ollama service only; no hosted fallback."],
        }
            ...
            payload.update(
                {
                    ...
                    "resolved_model_digest": (match or {}).get("digest"),
                    "model_size": (match or {}).get("size"),
                    "last_modified": (match or {}).get("modified_at"),
                    "model_family": details.get("family"),
                    "parameter_size": details.get("parameter_size"),
                    "quantization": details.get("quantization_level"),
                    "format": details.get("format"),
                    "context_length": settings_context(self.settings),
                }
            )
        except Exception:
    ...
    def _chat(
        ...
        body: dict[str, Any] = {
            ...
            "options": {
                ...
                "num_ctx": self.settings.local_planner_context_length,
                "num_predict": self.settings.local_planner_max_output_tokens,
            },
        ...
        for mode in modes:
            ...
            try:
                ...
                break
        raise LocalPlannerError(
            "request_timeout" if isinstance(last, httpx.TimeoutException) else "generation_failure",
            str(last or "Ollama request failed"),
            True,
        )
    ...
    def generate_cohort_plan(
        ...
        if any(pattern in lowered_request for pattern in UNSAFE_REQUEST_PATTERNS):
            raise LocalPlannerError(
                "prompt_injection_policy_failure", "request conflicts with planner safety policy"
            )
        ...
        user = json.dumps(
            {
                ...
                "request": request,
                "criteria": [item.model_dump(mode="json") for item in criteria or []],
        ...
        if not isinstance(content, str):
            raise LocalPlannerError(
                "malformed_response", "Ollama response did not contain message.content"
            )
        try:
            plan = CohortPlan.model_validate_json(content)
        except Exception as exc:
            repair_prompt = (
                f"Return only a valid CohortPlan JSON object. Validation error: {str(exc)[:1000]}"
            )
            repaired, repair_mode = self._chat(
                messages + [{"role": "user", "content": repair_prompt}], repair=True
            )
            repaired_content = repaired.get("message", {}).get("content")
            if not isinstance(repaired_content, str):
                raise LocalPlannerError(
                    "schema_validation_failure", "repair response was not JSON"
                ) from exc
            try:
                plan = CohortPlan.model_validate_json(repaired_content)
            except Exception as repair_exc:
                raise LocalPlannerError(
                    "schema_validation_failure", str(repair_exc)
                ) from repair_exc
        ...
        try:
            ...
            exc.lineage = {
                ...
                "schema_version": "cohort-plan-v1",
                "model_load_duration_ms": (response.get("load_duration") or 0) / 1_000_000,
                "prompt_evaluation_duration_ms": (response.get("prompt_eval_duration") or 0)
                / 1_000_000,
                "generation_duration_ms": (response.get("eval_duration") or 0) / 1_000_000,
        ...
        self._metadata.update(
            {
                "resolved_model_digest": health.get("resolved_model_digest")
                or response.get("model", self.model_name),
                "compatibility_mode": mode,
            }
        )
        return PlannerOutcome(
            plan,
            {
                **self.metadata(),
                ...
                "prompt_token_count": response.get("prompt_eval_count"),
                "generated_token_count": response.get("eval_count"),
                "repair_attempts": 1 if "repair" in mode else 0,
            },
        )
...
def validate_plan_output(plan: CohortPlan, dataset_id: str, max_candidates: int) -> None:
    ...
    if plan.dataset_id != dataset_id:
        raise LocalPlannerError(
            "schema_policy_violation", "planner dataset does not match the requested dataset"
        )
    ...
    for criterion in plan.criteria:
        if (
            criterion.verification_tool not in allowed_tools
            or criterion.verification_tool != TOOL_BY_TYPE[criterion.criterion_type]
        ):
            raise LocalPlannerError(
                "schema_policy_violation",
                "criterion verification tool is not allowlisted for its type",
            )
    if (
        not expected_tools.issubset(set(plan.required_tools))
        or "search_clinical_documents" not in plan.required_tools
    ):
...
class DeterministicCohortPlanner:
    ...
    def plan(
        ...
        if not selected:
            raise ValueError(
                "Request needs explicit supported criteria; no safe deterministic plan was found."
            )
        query = request.strip()
        return CohortPlan(
            ...
            required_tools=sorted(
                {TOOL_BY_TYPE[item.criterion_type] for item in selected}
                | {"search_clinical_documents"}
            ),
            verification_requirements=[item.criterion_id for item in selected],
    ...
    def _bounded_natural_language_plan(self, request: str) -> list[Criterion]:
        ...
        concepts = (
            ("hypertension", "condition", "condition-hypertension"),
            ("elevated blood pressure", "observation", "observation-blood-pressure"),
            ("blood pressure", "observation", "observation-blood-pressure"),
            ("colonoscopy", "procedure", "procedure-colonoscopy"),
            ("inhaler", "medication", "medication-inhaler"),
        )
        for concept, kind, criterion_id in concepts:
            if concept in lowered and not any(item.criterion_id == criterion_id for item in found):
                found.append(
                    Criterion(
                        criterion_id=criterion_id,
                        criterion_type=cast(CriterionType, kind),
...
def build_local_planner_provider(
    settings: Settings, model_name: str | None = None, benchmark: bool = False
