...
from app.observability.metrics import IDENTITY_AUTHZ, IDENTITY_DATASET, IDENTITY_REVIEW, observe
...
def _role_permissions(session: Session, role_name: str) -> frozenset[str]:
    rows = session.execute(
        select(RolePermission).join(Role, Role.id == RolePermission.role_id).where(Role.name == role_name)
    ).scalars()
    ...
    if not permission_ids:
        return frozenset()
    return frozenset(session.scalars(select(Permission.name).where(Permission.id.in_(permission_ids))).all())
...
def ensure_local_user(session: Session, settings: Settings, user_key: str) -> AuthenticatedUser:
    ...
    user = session.scalar(select(User).where(User.issuer == settings.identity_issuer, User.external_subject == user_key))
    if user is None:
        user = User(id=str(uuid4()), external_subject=user_key, issuer=settings.identity_issuer, display_name=display_name)
        session.add(user)
        session.flush()
    role = session.scalar(select(Role).where(Role.name == role_name))
    if role is None:
        raise HTTPException(status_code=503, detail="identity roles are not initialized")
    if session.scalar(select(UserRole).where(UserRole.user_id == user.id, UserRole.role_id == role.id)) is None:
    ...
    for dataset in datasets:
        grant = session.scalar(select(DatasetGrant).where(DatasetGrant.user_id == user.id, DatasetGrant.dataset_id == dataset.id))
        if grant is None:
            session.add(DatasetGrant(id=str(uuid4()), user_id=user.id, dataset_id=dataset.id, granted_by="local-bootstrap"))
        ...
        if role_name == "reviewer":
            assignment = session.scalar(select(ReviewerAssignment).where(ReviewerAssignment.user_id == user.id, ReviewerAssignment.dataset_id == dataset.id))
            if assignment is None:
                session.add(ReviewerAssignment(id=str(uuid4()), user_id=user.id, dataset_id=dataset.id))
...
def _to_auth_user(session: Session, user: User, role_name: str | None = None) -> AuthenticatedUser:
    role = role_name or session.scalar(select(Role.name).join(UserRole, UserRole.role_id == Role.id).where(UserRole.user_id == user.id))
    if not role:
        raise HTTPException(status_code=403, detail="identity has no active role")
    permissions = _role_permissions(session, role)
    dataset_ids = frozenset(session.scalars(select(DatasetGrant.dataset_id).where(DatasetGrant.user_id == user.id, DatasetGrant.enabled.is_(True))).all())
    return AuthenticatedUser(user.id, user.external_subject, user.issuer, user.display_name, user.email, role, permissions, dataset_ids, user.enabled)


def issue_session(user: AuthenticatedUser, settings: Settings) -> str:
    now = datetime.now(UTC)
    return jwt.encode(
        {"iss": settings.identity_issuer, "aud": settings.identity_audience, "sub": user.subject, "iat": now, "exp": now.timestamp() + settings.identity_session_ttl_seconds, "name": user.display_name},
...
def authenticate_request(request: Request, session: Session, settings: Settings) -> AuthenticatedUser:
    token = _token_from_request(request, settings)
    if not token:
        raise HTTPException(status_code=401, detail="authentication required")
    try:
        claims = jwt.decode(token, settings.identity_signing_secret, algorithms=["HS256"], audience=settings.identity_audience, issuer=settings.identity_issuer, options={"require": ["iss", "aud", "sub", "exp"]})
    except jwt.ExpiredSignatureError as exc:
        raise HTTPException(status_code=401, detail="session expired") from exc
    except jwt.InvalidTokenError as exc:
        raise HTTPException(status_code=401, detail="invalid session") from exc
    user = session.scalar(select(User).where(User.issuer == settings.identity_issuer, User.external_subject == str(claims["sub"])))
    if user is None or not user.enabled:
        raise HTTPException(status_code=401, detail="identity is disabled or unknown")
    return _to_auth_user(session, user)


def record_access(session: Session, user: AuthenticatedUser | None, action: str, resource_type: str, resource_id: str | None, decision: str, reason_code: str, correlation_id: str | None = None) -> None:
    trace = current_trace_context()
    session.add(AccessDecisionAudit(id=str(uuid4()), actor_id=user.internal_id if user else None, action=action, resource_type=resource_type, resource_id=resource_id, decision=decision, reason_code=reason_code, correlation_id=correlation_id, trace_id=trace.get("trace_id"), details={}))
    session.commit()


def require_permission(user: AuthenticatedUser, permission: str, session: Session, *, action: str = "authorization") -> None:
...
def require_dataset(user: AuthenticatedUser, dataset_id: str, session: Session, *, action: str = "dataset_access") -> None:
    from app.models.ingestion import Dataset
...
def require_reviewer(
    ...
    *,
    ...
    assignment = session.scalar(select(ReviewerAssignment).where(ReviewerAssignment.user_id == user.internal_id, ReviewerAssignment.dataset_id == dataset_id, ReviewerAssignment.enabled.is_(True)))
    if assignment is None:
