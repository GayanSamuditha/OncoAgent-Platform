...
def provider_for(settings: Settings, profile: str | None = None) -> DenseRetrievalProvider:
    ...
    if selected == "medcpt":
        return get_medcpt(
            settings.medcpt_query_model,
            settings.medcpt_document_model,
            settings.medcpt_query_revision,
            settings.medcpt_document_revision,
            settings.embedding_device,
            settings.retrieval_query_max_length,
            settings.retrieval_document_max_length,
        )
    if selected == "bioclinicalbert":
        return get_biobert(
            settings.clinical_embedding_model,
            settings.clinical_embedding_model_revision,
            settings.embedding_device,
...
def get_medcpt(
    ...
    return MedCPTDualEncoderProvider(
        query, document, query_revision, document_revision, device, query_length, document_length
    )
