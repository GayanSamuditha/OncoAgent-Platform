...
def summarize(results: list[dict[str, object]]) -> dict[str, float]:
    ...
    return {
        "precision_at_5": sum(float(item["precision_at_5"]) for item in values)
        / max(len(values), 1),
        "recall_at_5": sum(float(item["recall_at_5"]) for item in values) / max(len(values), 1),
        "mrr": sum(float(item["mrr"]) for item in values) / max(len(values), 1),
        "ndcg_at_5": sum(float(item["ndcg_at_5"]) for item in values) / max(len(values), 1),
        "zero_result_rate": sum(not bool(item["retrieved"]) for item in values)
        / max(len(values), 1),
        "median_query_latency_ms": median(latencies) if latencies else 0.0,
    }
