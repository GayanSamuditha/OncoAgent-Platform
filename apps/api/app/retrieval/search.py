...
def search(
    ...
    statement = (
        ...
        .join(
            ClinicalDocumentChunk, ClinicalDocumentChunk.id == ClinicalEmbedding.document_chunk_id
        )
        .join(ClinicalDocument, ClinicalDocument.id == ClinicalDocumentChunk.document_id)
        .where(
            ClinicalEmbedding.dataset_id == dataset_id,
            ClinicalEmbedding.provider_id == provider.metadata.provider_id,
            ClinicalEmbedding.document_model_revision == provider.metadata.document_model_revision,
        ...
        rows.append(
            {
                ...
                "encounter_id": embedding.encounter_id,
                "document_type": document.document_type,
                "text_excerpt": chunk.chunk_text[:600],
                "similarity_score": score,
                "source_fhir_resource_ids": chunk.source_resource_ids,
                "model_name": embedding.document_model_name,
                "model_revision": embedding.document_model_revision,
                "pooling_method": embedding.pooling_method,
                "document_builder_version": embedding.document_builder_version,
...
def last_indexing(session: Session, model_name: str) -> IndexingRun | None:
    return session.scalar(
        select(IndexingRun)
        .where(IndexingRun.model_name == model_name, IndexingRun.status == "completed")
...
def postgres_fts_search(
    ...
    statement = (
        select(ClinicalDocument, lexical_score)
        .where(
            ClinicalDocument.dataset_id == dataset_id,
            ClinicalDocument.document_type.in_(document_types),
            lexical_score > 0,
        )
        .order_by(lexical_score.desc(), ClinicalDocument.id)
        .limit(top_k)
    )
    if patient_id:
    ...
    rows = [
        {
            ...
            "source_fhir_resource_ids": doc.source_resource_ids,
            "model_name": "postgresql-fts",
            "model_revision": "database",
            "pooling_method": "lexical",
            "document_builder_version": doc.builder_version,
...
def reciprocal_rank_fusion(
    ...
    for rank, item in enumerate(lexical, 1):
        ...
        merged.setdefault(key, dict(item)).update(
            {"lexical_rank": rank, "lexical_score": item.get("similarity_score")}
        )
    for rank, item in enumerate(dense, 1):
        ...
        target.update(
            {
                "dense_rank": rank,
                "dense_similarity_score": item.get("similarity_score"),
    ...
    for item in merged.values():
        ...
        item["fused_score"] = (1 / (constant + lexical_rank) if "lexical_rank" in item else 0) + (
            1 / (constant + dense_rank) if "dense_rank" in item else 0
        )
    ranked = sorted(
        merged.values(),
        key=lambda item: (-float(cast(Any, item["fused_score"])), str(item["document_id"])),
    )[:limit]
...
def hybrid_search(
    ...
    started = time.perf_counter()
    lexical, _ = postgres_fts_search(
        session, dataset_id, query, candidate_pool, document_types, patient_id
    )
    dense, _ = search(
        session, provider, dataset_id, query, candidate_pool, document_types, patient_id, None
    )
    return reciprocal_rank_fusion(lexical, dense, rrf_constant, candidate_pool), (
