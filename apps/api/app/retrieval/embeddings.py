...
class _TransformerProvider:
    ...
    def _encode(self, texts: list[str], tokenizer: Any, model: Any, max_length: int, pooling: str) -> list[list[float]]:
        import torch
...
class MedCPTDualEncoderProvider(_TransformerProvider):
    def __init__(self, query_name: str, document_name: str, query_revision: str, document_revision: str, device: str, query_max_length: int = 64, document_max_length: int = 512) -> None:
        super().__init__(ProviderMetadata("medcpt", query_name, document_name, query_revision, document_revision, 768, select_device(device), POOLING_CLS, NORMALIZATION_L2, query_max_length, document_max_length))
    ...
    def load(self) -> None:
        if self.query_model is not None and self.document_model is not None:
            return
        try:
            self.query_tokenizer, self.query_model = self._load_model(self.metadata.query_model_name, self.metadata.query_model_revision)
            self.tokenizer, self.document_model = self._load_model(self.metadata.document_model_name, self.metadata.document_model_revision)
    ...
    def encode_queries(self, texts: list[str]) -> list[list[float]]:
        self.load()
        return self._encode(texts, self.query_tokenizer, self.query_model, self.metadata.query_max_length, POOLING_CLS)

    def encode_documents(self, documents: list[tuple[str, str]]) -> list[list[float]]:
        self.load()
        texts = [f"{title}\n{body}" for title, body in documents]
        return self._encode(texts, self.tokenizer, self.document_model, self.metadata.document_max_length, POOLING_CLS)
...
class BioClinicalBERTProvider(_TransformerProvider):
    ...
    def __init__(self, model_name: str, revision: str, device: str, max_length: int = 256) -> None:
        super().__init__(ProviderMetadata("bioclinicalbert", model_name, model_name, revision, revision, 768, select_device(device), POOLING_MEAN, NORMALIZATION_L2, max_length, max_length))

    def load(self) -> None:
        if self.model is not None:
            return
        try:
            self.tokenizer, self.model = self._load_model(self.metadata.document_model_name, self.metadata.document_model_revision)
        except Exception as exc:
            self.error = f"{type(exc).__name__}: {exc}"
            raise RuntimeError(self.error) from exc
    ...
    def encode_queries(self, texts: list[str]) -> list[list[float]]:
        self.load()
        return self._encode(texts, self.tokenizer, self.model, self.metadata.query_max_length, POOLING_MEAN)

    def encode_documents(self, documents: list[tuple[str, str]]) -> list[list[float]]:
        self.load()
        return self._encode([f"{title}\n{body}" for title, body in documents], self.tokenizer, self.model, self.metadata.document_max_length, POOLING_MEAN)
...
class DeterministicFakeProvider:
    def __init__(self, provider_id: str | int = "fake", dimension: int = 768) -> None:
        ...
        self.metadata = ProviderMetadata(provider_id, "fake-query", "fake-document", "test", "test", dimension, "cpu", POOLING_CLS, NORMALIZATION_L2, 64, 512)
        self.tokenizer = None
